...
public class TokenService {
    ...
    private String createToken(UserDetailsImpl userDetails, int ttlHours, TokenType tokenType) {
        ...
        return JWT.create()
                ...
                .withClaim(Claims.ID, userDetails.id().toString())
    ...
    public TokenPair createToken(Authentication authentication) {
        ...
        jwtStore.add(userDetails.id(), tokenPair);
        return tokenPair;
    }
    ...
    public TokenPair refreshToken(String refreshToken) {
        // @Override
        //    public TokenPair getAndGenerateNew(UUID uuid, final TokenPair tokenPair) {
        //        var res = template.opsForValue().getAndDelete(uuid.toString());
        //        if (res == null) {
        //            throw new AuthException(HttpStatus.NOT_FOUND, "Key does not exist");
        //        }
        //        tokenService.verify()
        //        add(uuid, tokenService.createToken());
        //        return res;
        //    }
    }
}
